...
import { baseUrl, getPatientById, getUserData, updatePatient } from '../../helpers/helpers'
...
const PatientProfile = () => {
  ...
  const onSubmit = (e) => {
    ...
    formData.append("data", JSON.stringify({...e,"ABHAHealthId":user.ABHAHealthId,"createdAt": user.createdAt,"password":user.password,"reportFile": user.reportFile,"updatedAt": user.updatedAt,"_v": user._v,"_id": user._id}));
    updatePatient(formData).then(res => {
      console.log(res,'res...')
      setModalData({...modalData, show: false})
      setTimeout(() => {
        setToastState({...toastState, show: true, type: "success", message: "Successfully updated"})
        getProfile()
      }, 300);
    }).catch(err => {
      console.log(err,'err...')
      setToastState({...toastState, show: true, type: "danger", message: "Failed to update"})

    })
  }

  useEffect(() => {
    const dataValues = _.omit(user,["ABHAHealthId","createdAt","password","reportFile","updatedAt","_v","_id"])
